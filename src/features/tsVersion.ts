...
// Memo: astro.useWorkspaceTsdk for coc-astro
function getConfigUseWorkspaceTsdk() {
  return workspace.getConfiguration('astro').get<boolean>('useWorkspaceTsdk', false);
}

// Memo: astro.tsLocale for coc-astro
function getConfigDiagnosticsTsLocale() {
  return workspace.getConfiguration('astro').get<string>('tsLocale', 'en');
}
